...
(function ($) {
		...
		prepareSingleCaption: function (obj) {
			...
			if (_nc.find("img").length > 0) {
				...
				if (_._responsive !== "off") {
					if (_._ba != "grid" && wful)
						if (jQuery.isNumeric(ww)) im.css({
							width: ww + "px"
						});
						else im.css({
							width: ww
						});
					else if (jQuery.isNumeric(ww)) im.css({
						width: ww * opt.bw + "px"
					});
					else im.css({
						width: ww
					...
					if (_._ba != "grid" && hful)
						if (jQuery.isNumeric(hh)) im.css({
							height: hh + "px"
						});
						else im.css({
							height: hh
						});
					else if (jQuery.isNumeric(hh)) im.css({
						height: hh * opt.bh + "px"
					});
					else im.css({
						height: hh
	...
	********************************************/
		createTimelineStructure: function (opt) {
			...
			function addTimeLineWithLabel(layer, opt, parentobject, slideid) {
				var timeline = new punchgs.TimelineLite({
					paused: true
				}),
					c;
				...
				parentobject[layer.attr("id")].triggerstate =
					layer.data("lasttriggerstate");
	...
	***************************************/
		...
		createFrameOnTimeline: function (obj) {
			...
			tweens.content = new punchgs.TimelineLite({
				align: "normal"
			});
			tweens.mask = new punchgs.TimelineLite({
				align: "normal"
			});
			...
			if (
				...
				timeline.add(tweens.mask.set(_._mw, {
					overflow: "visible"
				}), label);
...
**********************************************************************************************/
	...
	var tweenOnComplete = function (
		...
		if (_._nctype === "column") {
			punchgs.TweenLite.to(_._cbgc_man, 0.01, {
				visibility: "hidden"
			});
			punchgs.TweenLite.to(_._cbgc_auto, 0.01, {
				visibility: "visible"
			});
		}
		if (data.eventtype === "leftstage") {
