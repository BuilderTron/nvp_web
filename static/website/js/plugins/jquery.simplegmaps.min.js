...
(function ($) {
  ...
  function Plugin(element, options) {
    ...
    function init() {
      ...
      if (
        $(options.getRouteButton).length > 0 &&
        $(options.getTravelMode).length > 0
      ) {
        setupRouting(this);
      }

      if ($(options.geoLocationButton).length > 0) {
        $(options.geoLocationButton).on(
          "click",
          function (e) {
            e.preventDefault();
            geoLocation(Map.map);
    ...
    function drawMap() {
      ...
      if (
        $(options.SearchButton).length > 0 &&
        $(options.SearchBox).length > 0
      ) {
      ...
      // Else if routing button and input are present
        ...
        $(options.getRouteButton).length > 0 &&
        $(options.getFromAddress).length > 0
      ) {
      ...
      mapInData.find("div.map-marker").each(function (i) {
        if ($(this).attr("data-latlng")) {
          ...
          getMarkerIcon(
            currentMarkerData.data("icon"),
            currentMarkerData.data("icon2x"),
            function (iconMarker) {
              ...
              if (currentMarkerData.has("div.map-infowindow").length > 0) {
                var infowindow = new google.maps.InfoWindow({
                  content: currentMarkerData
                    .find("div.map-infowindow")
                    .parent()
                    .html(),
                });
                ...
                currentMarkerData.has("div.map-custom-infowindow").length > 0
              ) {
                var customInfowindow = currentMarkerData
                  .find("div.map-custom-infowindow")
                  .parent()
                  .html();
                google.maps.event.addListener(marker, "click", function () {
              ...
              if (status === google.maps.GeocoderStatus.OK) {
                getMarkerIcon(
                  ...
                  function (iconMarker) {
                    ...
                    if (
                      currentMarkerData.has("div.map-infowindow").length > 0
                    ) {
                      var infowindow = new google.maps.InfoWindow({
                        content: currentMarkerData
                          .find("div.map-infowindow")
                      ...
                      currentMarkerData.has("div.map-custom-infowindow")
                        .length > 0
                    ) {
                      var customInfowindow = currentMarkerData
                        .find("div.map-custom-infowindow")
                      ...
                      google.maps.event.addListener(
                        ...
                        function () {
                          $("#simplegmaps-c-iw").remove();
                          $('<div id="simplegmaps-c-iw"></div>').insertAfter(
                            $el
                          );
                          $("#simplegmaps-c-iw").html(customInfowindow);
                          $("#simplegmaps-c-iw .close").on(
                            "click",
                            function (event) {
                              event.preventDefault();
                              $("#simplegmaps-c-iw").remove();
    ...
    function guid() {
      var strguid = "xxxxxxxx-xxxx-4xxx-yxxx-xxxxxxxxxxxx".replace(
        /[xy]/g,
        function (c) {
