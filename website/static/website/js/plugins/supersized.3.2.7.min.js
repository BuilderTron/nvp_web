...
(function (e) {
	...
	e.supersized = function (t) {
		...
		r.init = function () {
			...
			e.supersized.vars.options = e.extend({},
		...
		r._build = function () {
			...
			while (t <= r.options.slides.length - 1) {
				...
				if (t == r.options.start_slide - 1) {
					...
					if (r.options.thumb_links) {
						r.options.slides[t].thumb ?
							(u = r.options.slides[t].thumb) :
							(u = r.options.slides[t].image);
						o =
							o +
							'<li class="thumb' +
							t +
					...
					r.options.slide_links &&
						(i = i + '<li class="slide-link-' + t + '" ><a>' + s + "</a></li>");
					if (r.options.thumb_links) {
						r.options.slides[t].thumb ?
							(u = r.options.slides[t].thumb) :
							(u = r.options.slides[t].image);
			...
			r.options.thumb_links &&
				...
				e(vars.thumb_tray).append(
					'<ul id="' + vars.thumb_list.replace("#", "") + '">' + o + "</ul>"
				);
			e(r.el).append(n);
			if (r.options.thumbnail_navigation) {
				vars.current_slide - 1 < 0 ?
					(prevThumb = r.options.slides.length - 1) :
					(prevThumb = vars.current_slide - 1);
				e(vars.prev_thumb)
					.show()
					.html(e("<img/>").attr("src", r.options.slides[prevThumb].image));
				vars.current_slide == r.options.slides.length - 1 ?
					(nextThumb = 0) :
					(nextThumb = vars.current_slide + 1);
				e(vars.next_thumb)
					.show()
					.html(e("<img/>").attr("src", r.options.slides[nextThumb].image));
			}
			r._start();
		};
		r._start = function () {
			r.options.start_slide ?
				(vars.current_slide = r.options.start_slide - 1) :
				(vars.current_slide = Math.floor(
					Math.random() * r.options.slides.length
				));
			var t = r.options.new_window ? ' target="_blank"' : "";
			r.options.performance == 3 ?
				r.$el.addClass("speed") :
				(r.options.performance == 1 || r.options.performance == 2) &&
				r.$el.addClass("quality");
			if (r.options.random) {
				arr = r.options.slides;
				for (
					var n, i, s = arr.length; s; n = parseInt(Math.random() * s),
					i = arr[--s],
					arr[s] = arr[n],
					arr[n] = i
				);
			...
			if (r.options.slides.length > 1) {
				if (r.options.slides.length > 2) {
					vars.current_slide - 1 < 0 ?
						(loadPrev = r.options.slides.length - 1) :
						(loadPrev = vars.current_slide - 1);
					var o = r.options.slides[loadPrev].url ?
						"href='" + r.options.slides[loadPrev].url + "'" :
						"",
			...
			if (r.options.slides.length > 1) {
				...
				o = r.options.slides[loadNext].url ?
					"href='" + r.options.slides[loadNext].url + "'" :
					"";
		...
		r.launch = function () {
			...
			if (r.options.slideshow && r.options.slides.length > 1) {
				r.options.autoplay && r.options.slides.length > 1 ?
					(vars.slideshow_interval = setInterval(
						r.nextSlide,
						r.options.slide_interval
					)) :
					(vars.is_paused = !0);
				...
				e("img", r.el).each(function () {
					function o(e) {
						if (e) {
							...
							if (
								n * t >= r.options.min_height ||
								(n * t >= r.options.min_height && t <= 1)
							) {
								thisSlide.width(n);
								...
								i <= r.options.min_height ?
									...
									i / n > t ?
										...
										r.options.fit_portrait && t >= 1 ?
											u() :
											o();
					thisSlide.parents("li").hasClass("image-loading") &&
						e(".image-loading").removeClass("image-loading");
					r.options.horizontal_center &&
						e(this).css("left", (n - e(this).width()) / 2);
					r.options.vertical_center &&
						e(this).css("top", (i - e(this).height()) / 2);
				});
		...
		r.nextSlide = function () {
			...
			vars.current_slide + 1 == r.options.slides.length ?
				(vars.current_slide = 0) :
				vars.current_slide++;
			var i = e(r.el + " li:eq(" + vars.current_slide + ")"),
				s = r.$el.find(".prevslide");
			r.options.performance == 1 &&
				r.$el.removeClass("quality").addClass("speed");
			loadSlide = !1;
			vars.current_slide == r.options.slides.length - 1 ?
				(loadSlide = 0) :
			...
			if (!e(o).html()) {
				...
				imageLink = r.options.slides[loadSlide].url ?
					"href='" + r.options.slides[loadSlide].url + "'" :
					"";
			...
			if (r.options.thumbnail_navigation == 1) {
				vars.current_slide - 1 < 0 ?
					(prevThumb = r.options.slides.length - 1) :
					(prevThumb = vars.current_slide - 1);
				e(vars.prev_thumb).html(
					e("<img/>").attr("src", r.options.slides[prevThumb].image)
				);
				nextThumb = loadSlide;
				e(vars.next_thumb).html(
					e("<img/>").attr("src", r.options.slides[nextThumb].image)
				);
			}
			typeof theme != "undefined" &&
				typeof theme.beforeAnimation == "function" &&
		...
		r._placeSlide = function (t) {
			...
			if (t == "next") {
				...
				vars.current_slide - 1 < 0 ?
					(loadSlide = r.options.slides.length - 1) :
					(loadSlide = vars.current_slide - 1);
				var i = r.el + " li:eq(" + loadSlide + ")";
				if (!e(i).html()) {
					var n = r.options.new_window ? ' target="_blank"' : "";
		...
		r.afterAnimation = function () {
			r.options.performance == 1 &&
				r.$el.removeClass("speed").addClass("quality");
			if (vars.update_images) {
				vars.current_slide - 1 < 0 ?
					(setPrev = r.options.slides.length - 1) :
